...
public class Life {
	 ...
	 * creates a 2D array of chars to store states of cells
	 */
	...
	//ansi color picker
	public static String printColor(char[][] board, int r, int c) {
	 ...
	 */
	public static void setCell(char[][] board, int r, int c, int[][] boardCount) {
		if (r >= 0 && r < board.length && c >= 0 && c < board[r].length) {
			if (boardCount[r][c] == 0) {
				board[r][c] = ' ';
			} else {
				board[r][c] = (char)boardCount[r][c];
			}
		}
	}//end method setCell
	 ...
	 * and return an int
	 *
	public static int[][] neighborCensus(char[][] board) {
	 ...
	 */
	public static boolean signsOfLife(char[][] board) {
		int count = 0;
		for (int r = 0; r < ROWS; r++) {
			for (int c = 0; c < COLS; c++) {
				if (board[r][c] != ' ') {
