...
public class Foogle
{
  ...
  //  foo method looks for an int value in an ArrayList
  // and returns the first index of (target) in (al)
  ...
  //  gizmo returns an ArrayList with numItems size
  // where each number is between lo, inclusive, and lo+hi, exclusive
  ...
  //  <YOUR DESCRIPTIVE COMMENT HERE> --- See comments below
  public static ArrayList wooHa(int numItems, int lo, int hi)
  {
    ArrayList retArr = new ArrayList();

    // error check to see if numItems is a valid number, but seems extraneous
    if ( numItems<1 )
      return retArr;

    // add an initial value to the ArrayList [0,lo)
    retArr.add( (int)(lo * Math.random()) );

    // creates the rest of the ArrayList so that each consecutive element is
	// increased from the previous element by a random number between
	// intial value, lo, and range, hi... [lo,lo+hi)
    for(int i=1; i<numItems; i++) {
      ...
      retArr.add( (int)retArr.get(i-1)
                  + lo + (int)(hi * Math.random()) ); //NOTE: The first (int) is necessary to cast the object to an int for calculating
    }
  ...
  public static void main( String[] args )
  {
    ...
    System.out.println("\n"+"sal03"+":");
    ArrayList sal03 = wooHa(5,0,100);  //repeat of sal01, consider changing arguments
